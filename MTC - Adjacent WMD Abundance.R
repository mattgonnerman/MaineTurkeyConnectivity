...
turkest <- read.csv("WMDTurkeyAbundance.csv") %>%
...
turkdens <- merge(wmdpoly, turkest, by = "WMD", all.x = T) %>%
...
for(i in c(1,5,6,7,8)){
  ...
  turkdens.out$JDen[which(turkdens.out$WMD == turkdens.na$WMD[i])] <- mean(temppoly$JDens, na.rm = T)
  turkdens.out$ADen[which(turkdens.out$WMD == turkdens.na$WMD[i])] <- mean(temppoly$ADens, na.rm = T)
